...
function customer_autocomplete(sl) {
    ...
    var options = {
        ...
        source: function (request, response) {
            ...
            var customer_cnpj = $('#customer_cnpj').val();
            var customer_ie = $('#customer_ie').val();
    ...
    // Aplicando o autocomplete para os três campos de busca
    $('body').on('keypress.autocomplete', '#customer_name, #customer_cnpj, #customer_ie', function () {
   ...
   var base_url = $("#base_url").val();
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#customer_address").text("Endereço: " + response.address);
            },
 ...
 function onselectimage(id){
         ...
         let get_loc = params.get('loc');
     
         var product_id = id;
         var product_table = get_loc;
         var base_url   = $('#base_url').val();
         var csrf_test_name = $('[name="csrf_test_name"]').val();
         ...
         if(product_id == exist){
           ...
           document.getElementById('add_item').focus();       
            ...
            $.ajax({
                ...
                data: {product_id: product_id,product_table: product_table,csrf_test_name:csrf_test_name},
                success: function (data) {
                    if (data == false) {
...
"use strict";
 $('body').on('keyup', '#product_name', function() {
        ...
        $.ajax({
            ...
            async: false,
            url: myurl,
            data: {product_name: product_name, product_table: product_table,csrf_test_name:csrf_test_name},
            success: function(data) {
                if (data == '420') {
    ...
    function check_category(category){
        ...
        var newUrl = window.location.pathname + "?loc=" + category;
        window.location.href = newUrl; // Redireciona e recarrega a página
    }
                    ...
                    // // Atualiza a URL sem recarregar a página
                    // var newUrl = window.location.pathname + "?loc=" + categoryName;
   ...
   $("#box-0").change(function(){
     ...
     if(checked){
       ...
       $(".cat-check").each(function(){
        ...
        // Obtém os parâmetros da URL
        let params = new URLSearchParams(window.location.search);
        let product_table = params.get('loc');
        console.log("Product Table:", product_table); // Exibe no console
