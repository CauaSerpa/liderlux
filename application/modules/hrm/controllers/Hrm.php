...
class Hrm extends MX_Controller {
    ...
    $this->form_validation->set_rules('email',display('email'),'required|max_length[255]');
   ...
   $data['employee'] = (object)$postData = [
        ...
        if ($this->form_validation->run()) {
            ...
            if (empty($id)) {
             if ($this->hrm_model->create_employee($postData)) { 
                ...
                $data['user_list']   = $this->hrm_model->user_list();
                $data['module']      = "hrm";  
     ...
     /*seller part start*/
      public function bdtask_seller_form($id = null){
    $data['title'] = display('add_seller');   
    $this->form_validation->set_rules('first_name',display('first_name'),'required|max_length[100]');
    $this->form_validation->set_rules('last_name',display('last_name'),'max_length[100]');
    $this->form_validation->set_rules('email',display('email'),'required|max_length[255]');
    $this->form_validation->set_rules('phone',display('phone'),'max_length[20]');
   ...
   $data['seller'] = (object)$postData = [
        ...
        if ($this->form_validation->run()) {
            ...
            if (empty($id)) {
             if ($this->hrm_model->create_seller($postData)) { 
                $this->session->set_flashdata('message', display('save_successfully'));
                 redirect("seller_list");
            } else {
                $this->session->set_flashdata('error_message',  display('please_try_again'));
                ...
                if ($this->hrm_model->update_seller($postData)) {
                    $old_head    =  $this->input->post('id',true).'-'.$this->input->post('old_first_name',true).''.$this->input->post('old_last_name',true);
                    $up_headname = $id.'-'.$data['first_name'].''.$data['last_name'];
                    $updatedby   = $this->session->userdata('id');
                    $updateddate = date('Y-m-d H:i:s');
                ...
                if(!empty($id)){
                $data['seller']    = $this->hrm_model->single_seller_data($id);
                $data['title']       = display('edit_seller');
                }
                $data['country_list']= $this->country_model->country();
                $data['desig']       = $this->hrm_model->designation_dropdown();
                $data['user_list']   = $this->hrm_model->user_list();
                $data['module']      = "hrm";  
                $data['page']        = "hrm/seller_form";  
                echo Modules::run('template/layout', $data);
            }
    }

        public function bdtask_seller_list(){
        $data['title']        = display('manage_seller');
        $data['employee_list']= $this->hrm_model->seller_list();
        $data['module']       = "hrm";
        $data['page']         = "hrm/seller_list"; 
        echo modules::run('template/layout', $data);
        }
