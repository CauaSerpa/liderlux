...
<span class="top-fixed-link" id="headerButtons">
      <?php
         
           if($this->permission1->method('new_invoice','create')->access()){
           ?>
           <a href="<?php echo base_url('add_invoice')?>" class="btn btn-success btn-outline"><i class="fa fa-balance-scale"></i> <span><?php  echo display('invoice') ?></span></a>
     <?php }?>

     
        <?php if($this->permission1->method('customer_receive','create')->access()){ ?>
           <a href="<?php echo base_url('customer_receive')?>" class="btn btn-success btn-outline"><i class="fa fa-money"></i> <span><?php echo display('customer_receive')?></span></a>
       <?php } ?>
      
  <?php if($this->permission1->method('supplier_payment','create')->access()){ ?>
          <a href="<?php echo base_url('supplier_payment')?>" class="btn btn-success btn-outline"><i class="fa fa-money" aria-hidden="true"></i> <span><?php echo display('supplier_payment')?></span></a>
      <?php } ?>

<?php if($this->permission1->method('add_purchase','create')->access()){ ?>
          <a href="<?php echo base_url('add_purchase')?>" class="btn btn-success btn-outline"><i class="ti-shopping-cart"></i> <span><?php echo display('purchase') ?></span></a>
 <?php } ?>
</span>
