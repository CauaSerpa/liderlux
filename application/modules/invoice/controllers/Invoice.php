...
class Invoice extends MX_Controller {
    ...
    public function bdtask_invoice_details($invoice_id = null){
        ...
        $enderecoCompleto = 
            $invoice_detail[0]['customer_address'] . ', ' . 
            $invoice_detail[0]['address_number'] . 
            (!empty($invoice_detail[0]['complement']) ? ' - ' . $invoice_detail[0]['complement'] : '') . ', ' .
            $invoice_detail[0]['zip'] . ' - ' .
            $invoice_detail[0]['city'] . '/' . $invoice_detail[0]['state'];

        $paytypes = [
            1 => display('cash_payment'),
            3 => display('deposit_payment'),
            4 => display('bill_payment'),
            5 => display('check_payment'),
            6 => display('pix_payment'),
            7 => display('voucher_payment'),
        ];
        
        $paytype_text = isset($paytypes[$invoice_detail[0]['payment_type']]) ? $paytypes[$invoice_detail[0]['payment_type']] : 'Não informado';
        ...
        'customer_address'  => $enderecoCompleto,
        'customer_mobile'   => $invoice_detail[0]['phone'],
        'customer_email'    => $invoice_detail[0]['customer_email'],
        'customer_code'     => $invoice_detail[0]['cod'],
        'customer_document' => $invoice_detail[0]['document'],
        ...
        'is_serial'         => $isserial,
        'is_unit'           => $isunit,
        'paytype'           => $paytype_text,
        'boleto_due_date'   => $invoice_detail[0]['boleto_due_date'],
        'observation'       => $invoice_detail[0]['observation'],
        );
        ...
        if (!empty($invoice_detail)) {
            ...
            foreach ($invoice_detail as $k => $v) {
                ...
                $invoice_detail[$k]['sl'] = $i;
                 ...
                 if(!empty($invoice_detail[$k]['product_id'])){
                    $isserial = $isserial+1;
    ...
    public function bdtask_edit_invoice($invoice_id = null){
        ...
        $data = array(
            ...
            'table'           => $invoice_detail[0]['invoice_table'],
            'invoice_details' => $invoice_detail[0]['invoice_details'],
            'invoice'         => $invoice_detail[0]['invoice'],
            'delivery_location'=> $invoice_detail[0]['delivery_location'],
            'expected_delivery_time'=> $invoice_detail[0]['expected_delivery_time'],
            'invoice_seller'  => $invoice_detail[0]['invoice_seller'],
            ...
            'total_discount_percent'=> $invoice_detail[0]['total_discount_percent'],
    ...
    public function bdtask_update_invoice(){
     $this->form_validation->set_rules('customer_id', display('customer_name') ,'required|max_length[15]');
    $this->form_validation->set_rules('paytype', display('payment_type') ,'required|max_length[20]');
    $this->form_validation->set_rules('product_table', display('product_table') ,'max_length[20]');
    $this->form_validation->set_rules('invoice_no', display('invoice_no') ,'required|max_length[20]');
    $this->form_validation->set_rules('product_id[]',display('product'),'required|max_length[20]');
    $this->form_validation->set_rules('product_quantity[]',display('quantity'),'required|max_length[20]');
    $this->form_validation->set_rules('product_rate[]',display('rate'),'required|max_length[20]');
    ...
    if ($this->form_validation->run() === true) {
       $invoice_id = $this->invoice_model->update_invoice();
        ...
        $data['details'] = $this->load->view('invoice/invoice_html_manual', $data, true);
        }else{
        $data['status'] = false;
        $data['exception'] = 'Please Try Again';
        }
    ...
    public function bdtask_duplicate_invoice($invoice_id = null)
    {
        ...
        // Chama o model para duplicar
        $new_invoice_id = $this->invoice_model->duplicate_invoice($invoice_id);
    
        if (!empty($new_invoice_id)) {
            // Redireciona diretamente para a nova fatura criada
            redirect('invoice_edit/' . $new_invoice_id);
        } else {
            echo "Falha ao duplicar a fatura.";
        }
    }

    public function bdtask_manual_sales_delete($invoice_id)
    {
        $data = array();
    
        if (empty($invoice_id)) {
            $data['status'] = false;
            $data['exception'] = 'ID da fatura não informado.';
            echo json_encode($data);
            return;
        }
    
        // Verifica se a fatura existe
        $invoice = $this->db->select('*')->from('invoice')->where('invoice_id', $invoice_id)->get()->row();
        if (empty($invoice)) {
            $data['status'] = false;
            $data['exception'] = 'Fatura não encontrada.';
            echo json_encode($data);
            return;
        }
        ...
        try {
            ...
            if ($this->db->trans_status() === FALSE) {
                throw new Exception('Erro ao deletar a fatura.');
            }
    
            $data['status'] = true;
            $data['message'] = 'Fatura excluída com sucesso.';
        ...
        // Redireciona sempre para a lista de invoices
        redirect('invoice_list');
    }
  ...
  public function bdtask_gui_pos(){
            $product_table     = $this->input->get('loc',TRUE);
            $taxfield = $this->db->select('tax_name,default_value')
                ->from('tax_settings')
                ->get()
                ->result_array();
            ...
            $data['itemlist']      = $this->invoice_model->allproduct($product_table);
     ...
     public function getitemlist_byname(){
            ...
            $product_table     = $this->input->post('product_table',TRUE);
            $getproduct       = $this->invoice_model->searchprod_byname($product_name, $product_table);
            if(!empty($getproduct)){
            $data['itemlist'] = $getproduct;
            $this->load->view('invoice/getproductlist', $data);  
            }else{
        ...
        $product_table = $this->input->post('product_table',TRUE);
        $product_details = $this->invoice_model->pos_invoice_setup($product_id, $product_table);
        $taxfield       = $this->db->select('tax_name,default_value')
                ->from('tax_settings')
                ->get()
                ->result_array();
        ...
        if (empty($serialdata)) {
            ...
            $tr .= "<tr id=\"row_" . $product_details->product_id . "\">
                        ...
                        <td>".$html."</td>
                        <td>
                            <input type=\"text\" name=\"available_quantity[]\" class=\"form-control text-right available_quantity_" . $product_details->product_id . "\" value='999999.00' readonly=\"\" id=\"available_quantity_" . $product_details->product_id . "\"/>
                        </td>
                        <td>
                        ...
                        <td class=\"\">
                            <input type=\"text\" name=\"discount[]\" onkeyup=\"quantity_calculate('" . $product_details->product_id . "');\" onchange=\"quantity_calculate('" . $product_details->product_id . "');\" id=\"discount_" . $product_details->product_id . "\" class=\"form-control text-right\" placeholder=\"0.00\" min=\"0\" readonly/>
    ...
    public function invoice_inserted_data_manual(){
        ...
        $customer_address  = "{$invoice_detail[0]['customer_address']}, {$invoice_detail[0]['address_number']}, {$invoice_detail[0]['complement']}";
        $customer_address_zip = "{$invoice_detail[0]['zip']}, {$invoice_detail[0]['city']} - {$invoice_detail[0]['state']}";
        
        $seller            = !empty($invoice_detail[0]['invoice_seller']) ? $invoice_detail[0]['invoice_seller'] : $this->session->userdata('fullname');
        $boleto_due_date   = !empty($invoice_detail[0]['boleto_due_date']) ? $invoice_detail[0]['boleto_due_date'] : '';
        ...
        'customer_address_zip'=> $customer_address_zip,
        'customer_mobile'   => $invoice_detail[0]['phone'],
        'customer_email'    => $invoice_detail[0]['customer_email'],
        'customer_code'     => $invoice_detail[0]['cod'],
        'customer_document' => $invoice_detail[0]['document'],
   ...
   /*invoice no generator*/
    ...
    public function bdtask_seller_autocomplete() {
        $seller_name = $this->input->post('seller_name', TRUE);
        $seller_info = $this->invoice_model->seller_search($seller_name); // Crie essa função no model
    
        $particles = ['da', 'de', 'do', 'dos', 'das', 'e'];
    
        $json_seller = [];
        foreach ($seller_info as $value) {
            $first_name = explode(' ', $value['first_name'])[0];
            $last_name_parts = explode(' ', $value['last_name']);
            $first_last_name = '';
            foreach ($last_name_parts as $part) {
                if (!in_array(strtolower($part), $particles)) {
                    $first_last_name = $part;
                    break;
                }
            }

            $json_seller[] = array(
                'label' => $value['first_name'] . " " . $first_last_name,
                'value' => $value['customer_id']
            );
        }
        echo json_encode($json_seller);
    }
 ...
 public function bdtask_customer_autocomplete(){
     ...
     /*after selecting product retrieve product info*/
        ...
        $product_id   = $this->input->post('product_id',TRUE);
        $product_table= $this->input->post('product_table',TRUE);
        $product_info = $this->invoice_model->get_total_product_invoic($product_id, $product_table);
    ...
    // Seleciona os dados do cliente incluindo documento e endereço
    $this->db->select("a.*, 
        b.HeadCode, 
        ((SELECT IFNULL(SUM(Debit),0) FROM acc_transaction WHERE COAID = b.HeadCode AND IsAppove = 1) - 
        (SELECT IFNULL(SUM(Credit),0) FROM acc_transaction WHERE COAID = b.HeadCode AND IsAppove = 1)) AS balance");
    $this->db->from('customer_information a');
    $this->db->join('acc_coa b', 'a.customer_id = b.customer_id', 'left');
    $this->db->where('a.customer_id', $customer_id);
    ...
    $document = !empty($result['document']) ? $result['document'] : 'N/A';
    $enderecoCompleto = 
        $result['customer_address'] . ', ' . 
        $result['address_number'] . 
        (!empty($result['complement']) ? ' - ' . $result['complement'] : '') . ', ' .
        $result['zip'] . ' - ' .
        $result['city'] . '/' . $result['state'];

    // Retorna os valores em formato JSON para o JavaScript manipular
    echo json_encode([
        'balance' => $balance,
        'document' => $document,
   ...
   $createdate = date('Y-m-d H:i:s');
    ...
    $customer_coa = [
        ...
        $paytype_text = isset($paytypes[$invoice_detail[0]['paytype']]) ? $paytypes[$invoice_detail[0]['paytype']] : display('cash_payment');
