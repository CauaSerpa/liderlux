...
$total_customer = isset($total_customer) ? $total_customer : 0;

if ($todays_sales_report === false) {
    echo "Error";
}
?>
        ...
        <?php }?>
            ...
            <a href="<?php echo base_url('product_list') ?>" class="small-box-footer"><?php echo display('total_product_table_liderlux')?></a>
          </div>
            </div>
        <?php }?>
             <?php if($this->permission1->method('manage_product_sp','read')->access()){ ?>
            <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
                <div class="small-box bg-pase whitecolor">
            <div class="inner">
              <h4><span class="count-number"><?php echo html_escape($total_product_sp) ?></span></h4>

              <p><?php echo display('total_product_sp')?></p>
            </div>
            <div class="icon">
             <i class="fa fa-shopping-bag"></i>
            </div>
            <a href="<?php echo base_url('product_list_sp') ?>" class="small-box-footer"><?php echo display('total_product_table_sp')?></a>
          </div>
            </div>
        <?php }?>
             <?php if($this->permission1->method('manage_product_rj','read')->access()){ ?>
            <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
                <div class="small-box bg-pase whitecolor">
            <div class="inner">
              <h4><span class="count-number"><?php echo html_escape($total_product_rj) ?></span></h4>

              <p><?php echo display('total_product_rj')?></p>
            </div>
            <div class="icon">
             <i class="fa fa-shopping-bag"></i>
            </div>
            <a href="<?php echo base_url('product_list_rj') ?>" class="small-box-footer"><?php echo display('total_product_table_rj')?></a>
          </div>
            </div>
        <?php }?>
             <?php if($this->permission1->method('manage_product_br','read')->access()){ ?>
            <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
                <div class="small-box bg-pase whitecolor">
            <div class="inner">
              <h4><span class="count-number"><?php echo html_escape($total_product_br) ?></span></h4>

              <p><?php echo display('total_product_br')?></p>
            </div>
            <div class="icon">
             <i class="fa fa-shopping-bag"></i>
            </div>
            <a href="<?php echo base_url('product_list_br') ?>" class="small-box-footer"><?php echo display('total_product_table_br')?></a>
          </div>
            </div>
        <?php }?>
        </div>
                    ...
                    <div class="panel panel-bd">
                        <div class="panel-heading">
                            <div class="panel-title">
                                ...
                                <a href="<?php echo base_url(); ?>dashboard/home/see_all_best_sales" class="btn btn-success text-white best-sale-seeall">Ver Tudo</a>
                            </div>
                        </div>
    ...
    <?php
    if (isset($chart_data) && is_string($chart_data)) {
        $separatedData = explode(',', $chart_data);
        $firstValue = isset($separatedData[0]) ? (int)$separatedData[0] : 0;
        $safeValue = htmlspecialchars($firstValue + 10, ENT_QUOTES, 'UTF-8');
    } else {
        $safeValue = 0;
    }
    ?>
    <input type="hidden" value="<?php echo $safeValue; ?>" name="" id="bestsalemax">
            </div>
        <?php }?>
