   ...
   // category part
    ...
    public function bdtask_product_form($id = null){
       $data['title'] = display('add_product');
        ...
        $this->form_validation->set_rules('category_id',display('category'),'max_length[20]');
        $this->form_validation->set_rules('price', display('price') ,'max_length[12]');
        $this->form_validation->set_rules('table', display('table') ,'max_length[50]');
        ...
        $data['product'] = (object)$postData = [
            ...
            'price'        => !empty($id) ? $this->input->post('price',TRUE) : null,
            'table'        => !empty($id) ? $this->input->post('table',TRUE) : null,
            'product_model'=> $this->input->post('model',TRUE),
       ...
       foreach ($txf as $key => $value) {
        ...
        if ($this->form_validation->run() === true) {
            ...
            #if empty $id then insert data
            if (empty($id)) {
                // Pegando os preços por região
                $price_sp = $this->input->post('price_sp', TRUE);
                $price_rj = $this->input->post('price_rj', TRUE);
                $price_br = $this->input->post('price_br', TRUE);
            
                // Pelo menos um preço obrigatório
                if (empty($price_sp) && empty($price_rj) && empty($price_br)) {
                    $this->session->set_flashdata('exception', 'Pelo menos um preço deve ser informado.');
                ...
                foreach ($regions as $region => $price_value) {
                    if (!empty($price_value)) {
                        $postDataRegion = $postData;

                        $postDataRegion['table'] = $region;
                        $postDataRegion['price'] = $price_value;

                        $this->product_model->create_product($postDataRegion);
                    }
                }

                $this->session->set_flashdata('message', display('save_successfully'));
                redirect("product_list"); // pode escolher padrão
        ...
        //         }
                ...
                //         break;
                //     case 'SP':
                //         redirect("product_list_sp");
                //         break;
                //     case 'RJ':
                //         redirect("product_list_rj");
                //         break;
                //     case 'BR':
                //     default:
                //         redirect("product_list_br");
                //         break;
                // }
            } else {
                $postData['id'] = $id;
    ...
    public function bdtask_product_list_sp(){
        $data['title']         = display('manage_product_sp');
        $data['total_product'] = $this->db->where("`table`", "SP")->count_all_results("product_information");
        $data['module']        = "product";
        $data['page']          = "product_list_sp"; 
        echo modules::run('template/layout', $data);
    }

    public function bdtask_product_list_rj(){
        $data['title']         = display('manage_product_rj');
        $data['total_product'] = $this->db->where("table", "RJ")->count_all_results("product_information");
        $data['module']        = "product";
        $data['page']          = "product_list_rj"; 
        echo modules::run('template/layout', $data);
    }

    public function bdtask_product_list_br(){
        $data['title']         = display('manage_product_br');
        $data['total_product'] = $this->db->where("table", "BR")->count_all_results("product_information");
        $data['module']        = "product";
        $data['page']          = "product_list_br"; 
        echo modules::run('template/layout', $data);
    }
        ...
        public function CheckProductListSP(){
        $postData = $this->input->post();
        $data = $this->product_model->getProductListSP($postData);
        echo json_encode($data);
    } 
        public function CheckProductListRJ(){
        $postData = $this->input->post();
        $data = $this->product_model->getProductListRJ($postData);
        echo json_encode($data);
    } 
        public function CheckProductListBR(){
        $postData = $this->input->post();
        $data = $this->product_model->getProductListBR($postData);
        echo json_encode($data);
    } 
    ...
    public function bdtask_deleteproduct($delete_id = null) {
        $id = $this->db->select('product_id')
        ...
        if ($check_calculation > 0) {
            $table = $this->db->select('table')
                              ->from('product_information')
                              ->where('product_id', $id)
                              ->get()
                              ->row('table');
    
            $this->session->set_flashdata('exception', 'You cannot delete this product because it is already engaged in a calculation.');
    ...
    function uploadCsv()
    {
        ...
        public function qrgenerator($product_id_url) {
        $product_id = rawurldecode($product_id_url);
    ...
    // bar code part
    public function barcode_print($product_id_url){
        $product_id = rawurldecode($product_id_url);
        $product_info = $this->product_model->bdtask_barcode_productdata($product_id);
...
public function bdtask_product_details($product_id_url = null){
        $product_id = rawurldecode($product_id_url);
